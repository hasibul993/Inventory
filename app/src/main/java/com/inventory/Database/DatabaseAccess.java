...
public class DatabaseAccess extends DatabaseHelper {
    ...
    public void InsertUpdateSettings(SettingsModel settingsModel) {
        ...
        try {
            ...
            if (_id == -1) {
                db.update(TABLE_SETTINGS_DB, values, COLUMN_USERGUID + "= '" + settingsModel.UserGuid + "'", null);
            }
            ...
            for (DrugModel drugModel : drugModelArrayList) {
                try {
                    ...
                    if (drugModel.DrugExpiryDate != null) {
                        ...
                        values.put(COLUMN_EXPIRY_DATE_IN_MILLISECOND, mainActivity.GetMilliSecondsFromDate(drugModel.DrugExpiryDate));
                    }
                    ...
                    if (_id == -1) {
                        ...
                        // db.update(TABLE_INVENTORY_DB, values, COLUMN_DRUG_ID + "= '" + drugModel.DrugID + "'", null);
    ...
    public void InsertDrugsInBatchInPharmacyDB(ArrayList<DrugModel> drugModelArrayList) {
        SQLiteDatabase db = super.getWritableDatabase();
        db.beginTransaction();
        ContentValues values = new ContentValues();
        try {
            ...
            if (cursor.moveToFirst()) {
                while (!cursor.isAfterLast()) {
                    try {
                        ...
                        drugModel.ExpiryDateInMilliSecond = cursor.getLong(cursor.getColumnIndex(COLUMN_EXPIRY_DATE_IN_MILLISECOND));
                        //String date=mainActivity.GetDateFromMilliseconds(drugModel.ExpiryDateInMilliSecond );
    ...
    public ArrayList<DrugModel> GetDrugManufacturerListFromMasterDB(String searchText) {
        SQLiteDatabase db = super.getWritableDatabase();
        Cursor cursor = null;
        ArrayList<DrugModel> drugModelArrayList = new ArrayList<>();
        ...
        try {
            ...
            if (cursor.getCount() == 0)
                cursor = db.rawQuery(DatabaseQuery.GetQueryForSearchDrugManufacturerInPharmacyDB(searchText), null);
            ...
            ex.printStackTrace();
            Log.i(TAG, " GetDrugManufacturerListFromMasterDB() : " + ex.getMessage());
        }
